...
class MC(commands.Cog):
    ...
    async def con(self, ctx, username: str, password: str):
        ...
        if hashed_pw != db_password:    # If the password is incorrect
            await msg.edit_original_response(embed=discord.Embed(title="Connect MC Account", description="*Incorrect Password!*", color=discord.colour.Color.red()).set_footer(text=embed_footer).set_author(name=embed_header, icon_url=embed_icon))
            return
        ...
        await msg.edit_original_response(embed=con_embed)   # Edit the message to show the success message
    ...
    async def changepassword_mc(self, ctx, password: str):
        ...
        await emb.edit_original_response(embed=pw_embed)
    ...
    async def pay(self, ctx, user: discord.Member, amount: int):
        ...
        await emb.edit_original_response(embed=embd)
