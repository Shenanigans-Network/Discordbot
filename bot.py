...
""""
async def logger(ctx, cat, printmsg, logmsg):
    print(printmsg)
    logmsg.replace('{countadd(cat)}', '{countadd(' + cat + ")}")
    await log_channel.send(logmsg)


@client.command()
async def test(ctx):    await logger(ctx, "a", "thisis a print msg", f"this is a {countadd()} message")
"""
...
async def ip_embed(ctx):
    ...
    print(f'Sent IP Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Console
    cat = "i"
    await log_channel.send(f"**Info** : #{countadd(cat)} Sent IP embed to message of {ctx.author.name}#{ctx.author.discriminator}")
...
async def version_embed(ctx):
    ...
    cat = "i"
    await log_channel.send(f"**Info** : #{countadd(cat)} Sent Version embed to message of {ctx.author.name}#{ctx.author.discriminator}")
...
async def on_message(ctx):  # On message, Checks every message for...
    if ctx.author.bot:
        ...
        if " ip " in f" {ctx.content} ": await ip_embed(ctx)  # On word IP send ip embed
            ...
            print(f"Sent Mention message to {ctx.author.name}#{ctx.author.discriminator}")
            cat = "h"
            await log_channel.send(f"**Help** : #{countadd(cat)} Sent mention-message to message of {ctx.author.name}#{ctx.author.discriminator}")

        # Check Messages for [servername] and "Down/up" A repeat of the same thing multiple times for every server
        elif " survival " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "survival", "192.168.100.80:25575")
        elif " skyblock " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "skyblock", "192.168.100.70:25572")
        elif " duels " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "duels", "192.168.100.70:25573")
        elif " bedwars " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "bedwars", "192.168.100.70:25571")
        elif " auth " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "auth", "192.168.100.70:25578")
        elif " proxy " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "proxy", "192.168.100.60:25565")
        elif " lobby " in f" {ctx.content} " and "down" in f" {ctx.content} ":
            await serverstatus(ctx, "lobby", "192.168.100.70:25577")

        # Check msgs for [servername] and "Up"
        elif " survival " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "survival", "192.168.100.80:25575")
        elif " skyblock " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "skyblock", "192.168.100.70:25572")
        elif " duels " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "duels", "192.168.100.70:25573")
        elif " bedwars " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "bedwars", "192.168.100.70:25571")
        elif " auth " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "auth", "192.168.100.70:25578")
        elif " proxy " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "proxy", "192.168.100.60:25565")
        elif " lobby " in f" {ctx.content} " and "up" in f" {ctx.content} ":
            await serverstatus(ctx, "lobby", "192.168.100.70:25577")
...
async def status_bothelp(ctx):
    ...
    print(
        f'Help : Sent Status-Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Console
...
async def resetcounter_admin(ctx):
    ...
    print(f'Help : Sent Admin-resetcounter Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Console
...
async def changeserverpower_admin(ctx):  # Help changeserverstate
    ...
    print(f'Help : Sent Admin-css Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Console
...
async def changecmd_admin(ctx):  # Help sendcmd
    ...
    await ctx.reply(embed=bothelp)
    cat = "a"
    await log_channel.send(f'**Help** : #{countadd(cat)} Sent Admin Change-cmd sub-Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Log channel
    print(f'Help : Sent Admin-cmd Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Console
...
async def password_admin(ctx):  # Help password
    ...
    await ctx.reply(embed=bothelp)
    cat = "a"
    await log_channel.send(f'**Help** : #{countadd(cat)} Sent Admin Change-pw sub-Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Log channel
    print(f'Help : Sent Admin-pw Help Embed to message of {ctx.author.name}#{ctx.author.discriminator}')  # Logs to Console
...
async def embed(ctx, *data):
    ...
    await log_channel.send(
        f"**Fun** : #{countadd(cat)} Sent embed-creator embed to message of {ctx.author.name}#{ctx.author.discriminator}")  # Logging to log channel
...
async def sendcmd_admin(ctx, *data):    # Send Command Admin Command
    ...
    embed.set_footer(text=f"{embed_footer}")
    await ctx.reply(embed=embed)
    print(f"{ctx.author.name}#{ctx.author.discriminator} Sent Command")    # Logs to Console
    await log_channel.send(f'**Admin** : #{countadd("a")} Sent command to `{data[0]}`')  # Logs to Log channel
